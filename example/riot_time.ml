...
let sleep_ns d =
  ...
  let t_nano = Monotonic.(time () * 1000L) in
  let t = Monotonic.(t_nano + d) in
...
let rec restart_threads now =
  ...
  | Some (_, { canceled = true; _ }) ->
      ...
      restart_threads now
  | Some (_, { time; thread; _ }) when in_the_past now time ->
      sleep_queue := unpack (SleepQueue.rest !sleep_queue);
      Lwt.wakeup thread ();
      restart_threads now
  | _ -> ()
...
let select_next () =
  ...
  sleep_queue :=
    List.fold_left
      (fun q e -> SleepQueue.add (new_id ()) e q)
      !sleep_queue !new_sleeps;
  new_sleeps := [];
  get_next_timeout ()

module S : Mirage_time.S = struct
  let sleep_ns t = sleep_ns t
end
